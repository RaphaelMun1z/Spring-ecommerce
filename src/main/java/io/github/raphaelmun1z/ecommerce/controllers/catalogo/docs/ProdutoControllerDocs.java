...
public interface ProdutoControllerDocs {
    ...
    ResponseEntity<Page<ProdutoResponseDTO>> listarTodos(@Parameter(hidden = true) Pageable pageable);

    @Operation(summary = "Vitrine de Produtos", description = "Retorna apenas produtos ativos e com estoque.")
    @ApiResponse(responseCode = "200", description = "Lista recuperada com sucesso")
    ResponseEntity<Page<ProdutoResponseDTO>> listarAtivos(@Parameter(hidden = true) Pageable pageable);
    ...
    ResponseEntity<ProdutoResponseDTO> buscarPorId(@Parameter(description = "ID do produto") @PathVariable String id);
    ...
    ResponseEntity<ProdutoResponseDTO> criar(@RequestBody @Valid ProdutoRequestDTO dto);

    @Operation(summary = "Atualizar produto")
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Produto atualizado com sucesso", content = @Content(schema = @Schema(implementation = ProdutoResponseDTO.class))),
        @ApiResponse(responseCode = "404", description = "Produto não encontrado", content = @Content)
    })
    ResponseEntity<ProdutoResponseDTO> atualizar(@Parameter(description = "ID do produto") @PathVariable String id, @RequestBody @Valid ProdutoRequestDTO dto);

    @Operation(summary = "Excluir produto (Físico)")
    @ApiResponse(responseCode = "204", description = "Produto excluído com sucesso")
    ResponseEntity<Void> excluir(@Parameter(description = "ID do produto") @PathVariable String id);
